...
raup_crick_modified=function(spXsite,
                             nocore=2,
                             reps=9999,
                             filename="all_sample"){
  
  ##spXsite: a species by site matrix, with row names for plots.  
  ##Note that the choice of how many plots (rows) to include has a real impact on the metric, as species and their occurrence frequencies across the set of plots is used to determine gamma and the frequency with which each species is drawn from the null model
  library(vegan)
  library(tidyr)
  library(snowfall)
  ...
  ##make the spXsite matrix into a pres/abs. (overwrites initial spXsite matrix):
  spXsite_standard <- decostand(spXsite,"pa")
  ##create an occurrence vector- used to give more weight to widely distributed species in the null model:
  occur <- colSums(spXsite_standard)
  
  ## determine how many unique species richness values are in the dataset
  ##this is used to limit the number of null communities that have to be calculated
  alpha_levels<-sort(unique(rowSums(spXsite_standard)))
  
  #generate the matrix for the input of func_subRC function
  mtr <- as.data.frame(matrix(0,length(alpha_levels),length(alpha_levels)))
  rownames(mtr) <- colnames(mtr) <- 1:length(alpha_levels)
  mtr[upper.tri(mtr)] <- NA
  mtr$id2 <- rownames(mtr)
  long_bray <- gather(mtr,key = "id1", value = "repsno",na.rm = T,-id2)
  ...
  func_subRC <- function(nl,vfsmmy,spXsite,alpha_levels,occur){
    #library(snowfall)
    #library(vegan)	 
    a1 <- vfsmmy[nl,1]
    a2 <- vfsmmy[nl,2]
    ...
    #finished_tmp <- nl
    #write.table(finished_tmp,file = paste0(stt,"/",nl,"_finished.txt"),quote = F,row.names = F,col.names = F,sep = "\t")
    ...
    output <- c(alpha_levels[a1],alpha_levels[a2],n,dis2)
    
    return(output)
  }
  
  
  #stt <- paste0("finished_",nrow(vfsmmy))
  #if(file.exists(stt)){unlink(stt, recursive = T, force = T)}
  #dir.create(stt)
  ...
  sfExport("vfsmmy","spXsite", "alpha_levels", "occur")
  results <- sfLapply(1:nrow(vfsmmy),func_subRC,vfsmmy <- vfsmmy, spXsite <- spXsite,alpha_levels<-alpha_levels,occur <- occur)
  sfStop()
  
  null_shared_spp <- matrix(unlist(results), nrow=4)
  null_shared_spp <- null_shared_spp[,order(null_shared_spp[2,])]
  null_shared_spp <- null_shared_spp[,order(null_shared_spp[1,])]
  null_shared_spp <- null_shared_spp[,order(null_shared_spp[3,])]
  
  print("Parallel processes finished")
  
  ##build a site by site matrix for the results, with the names of the sites in the row and col names:
  lastresults<-matrix(data=NA, nrow=n_sites, ncol=n_sites, dimnames=list(row.names(spXsite), row.names(spXsite)))
  
  ##for each pair of sites (duplicates effort now to make a full matrix instead of a half one- but this part should be minimal time as compared to the null model building)
  
  #mtr2 <- as.data.frame(matrix(0,n_sites,n_sites))
  #rownames(mtr2) <- colnames(mtr2) <- 1:n_sites
  mtr2 <- as.data.frame(lastresults)
  mtr2[,] <- 0
  mtr2[upper.tri(mtr2)] <- NA
  mtr2$id1 <- rownames(mtr2)
  longmtr <- gather(mtr2,key = "id2", value = "rn",na.rm = T,-id1)
  
  func_mtr <- function(nlm, longmtr, reps,spXsite, spXsite_standard,null_shared_spp){
    ##how many species are shared between the two sites:
    n_shared_obs <- vegdist(spXsite[c(longmtr[nlm,1],longmtr[nlm,2]),],method="bray")
    
    ## what was the observed richness of each site?
    obs_a1<-sum(spXsite_standard[longmtr[nlm,1],])
    obs_a2<-sum(spXsite_standard[longmtr[nlm,2],])
    
    ##place these alphas into an object to match against alpha_table (sort so smaller alpha is first)
    obs_a_pair<-sort(c(obs_a1, obs_a2))
    
    nullrc <- null_shared_spp[4,null_shared_spp[1,]==obs_a_pair[1] & null_shared_spp[2,]==obs_a_pair[2]]
    ...
    return(c(longmtr[nlm,1],longmtr[nlm,2],round(rc, digits=3)))
  }
  
  sfInit(parallel = TRUE, cpus = nocore)
  sfLibrary(vegan)
  
  sfExport("longmtr","spXsite","reps", "spXsite_standard", "null_shared_spp")
  
  rcres <- sfLapply(1:nrow(longmtr),func_mtr,
                    longmtr <- longmtr, 
                    reps <- reps,
                    spXsite <- spXsite,
                    spXsite_standard<-spXsite_standard,
                    null_shared_spp <- null_shared_spp)
  
  sfStop()
  
  longRC <- as.data.frame(t(matrix(unlist(rcres), nrow=3)))
  
  longRCs <- longRC[longRC$V1 != longRC$V2,]
  longRCs <- longRCs[,c(2,1,3)]
  colnames(longRCs) <- colnames(longRC)
  longRC <- rbind(longRC,longRCs)
  
  longRC[,1] <- as.character(longRC[,1])
  longRC[,2] <- as.character(longRC[,2])
  longRC[,3] <- as.character(longRC[,3])
  
  lastresults <- pivot_wider(longRC,names_from = V2,values_from = V3)
  
  #unlink(stt, recursive = T, force = T)
  
  write.table(lastresults,file = paste(filename,"RCbray.txt",sep = "_"),quote = F,row.names = F,col.names = T,sep = "\t")
}
