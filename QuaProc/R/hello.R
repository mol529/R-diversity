...
betaNTI <- function(com,
  ...
  if(nbin == 1){
    ...
    rand.weighted.bMNTD.comp <- big.matrix(nrow=beta.reps*nr, ncol=3, init=-999, backingfile=paste0('parall_bnti_',filename,'_null_model_res.bin'), descriptorfile=paste0('parall_bnti_',filename,'_null_model_res.desc'))
...
raup_crick_modified=function(comm,
  ...
  if(nbin == 1) {
    ...
    null_dist <- big.matrix(nrow=3, ncol=ttl_reps, init=-999, backingfile=paste0('parall_rcbray_',filename,'_null_model_res.bin'), descriptorfile=paste0('parall_rcbray_',filename,'_null_model_res.desc'))
